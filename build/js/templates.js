angular.module('coachSeekApp').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('workingHours/partials/coachListView.html',
    "<a href=\"#/registration/locations\">{{'workingHours:nav-to-locations' | i18next}}</a><h3>{{'workingHours:coach-list-title' | i18next}}</h3><a href=\"#/registration/services\">{{'workingHours:nav-to-services' | i18next}}</a><div class=\"coach-list-view\" ng-hide=\"coach\"><ul><li class=\"coach-details\" ng-repeat=\"coach in coachList | orderBy:'lastName'\"><span class=\"coach-name\">{{coach.firstName}} {{coach.lastName}}</span> <span class=\"coach-email\">{{coach.email}}</span> <span class=\"coach-phone\">{{coach.phone}}</span><!-- show coach edit on click --> <button class=\"edit-coach\" ng-click=\"editCoach(coach)\">{{'edit' | i18next}}</button></li></ul><!-- show coach creation on click --><button ng-click=\"createCoach()\">{{'workingHours:add-new-coach' | i18next}}</button></div><div class=\"coach-edit-view\" ng-show=\"coach\"><form>{{'person-details.first-name' | i18next}}<input ng-model=\"coach.firstName\"> {{'person-details.last-name' | i18next}} <input ng-model=\"coach.lastName\"> {{'person-details.email' | i18next}} <input ng-model=\"coach.email\"> {{'person-details.phone' | i18next}} <input ng-model=\"coach.phone\"><time-slot></time-slot><!-- POST here --><button class=\"save-coach\" ng-click=\"save(coach)\">{{'save' | i18next}}</button></form></div>"
  );
